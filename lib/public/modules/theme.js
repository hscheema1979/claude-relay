...
function themeIdForMode(mode) {
  ...
  // Default skin pair: claude (dark) / claude-light (light)
  if (!skin) {
    return mode === "light" ? "claude-light" : "claude";
  }

  // Custom skin — try to find the counterpart
  var current = getTheme(skin);
  if (!current) return mode === "light" ? "claude-light" : "claude";
...
export function initTheme() {
  ...
  if (saved) {
    ...
    currentThemeId = mode === "light" ? "claude-light" : "claude";
  }
