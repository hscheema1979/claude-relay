...
// --- DOM refs ---
  ...
  function startRateLimitCountdown(el, resetsAt, cardEl) {
    ...
    function tick() {
      ...
      if (remaining <= 0) {
        ...
        if (cardEl) cardEl.classList.add("rate-limit-cleared");
  ...
  function updateRateLimitIndicator(msg) {
    ...
    rateLimitIndicatorEl.className = "header-rate-limit" + (isRejected ? " rejected" : " warning");
    rateLimitIndicatorEl.innerHTML = iconHtml("alert-triangle") + '<span class="header-pill-text">' + (isRejected ? "Rate limited" : "Rate warning") + "</span>";
    refreshIcons();
  }
  ...
  function handleRateLimitEvent(msg) {
    var isRejected = msg.status === "rejected";
    var div = document.createElement("div");
    div.className = "rate-limit-msg" + (isRejected ? " rate-limit-rejected" : "");

    var header = document.createElement("div");
    header.className = "rate-limit-header";
    header.textContent = isRejected ? "Rate limit reached" : "Approaching rate limit";
    div.appendChild(header);
    ...
    if (isRejected && msg.resetsAt) {
      // Check if already expired (history replay)
      if (msg.resetsAt < Date.now()) {
        hint.textContent = typeLabel + " limit exceeded. Rate limit has since cleared.";
        div.classList.add("rate-limit-cleared");
      } else {
        var countdownSpan = document.createElement("span");
        countdownSpan.className = "rate-limit-countdown";
        hint.textContent = typeLabel + " limit exceeded. Resets in ";
        hint.appendChild(countdownSpan);
        hint.appendChild(document.createTextNode("."));
        startRateLimitCountdown(countdownSpan, msg.resetsAt, div);
        updateRateLimitIndicator(msg);
      }
    } else {
      var pct = msg.utilization ? Math.round(msg.utilization * 100) : null;
      hint.textContent = typeLabel + " limit warning" + (pct ? " (" + pct + "% used)" : "") + ". Consider slowing down to avoid interruption.";
      updateRateLimitIndicator(msg);
    }
    div.appendChild(hint);
