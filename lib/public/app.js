...
// --- DOM refs ---
  ...
  var wizardStep = 1;
  var wizardData = { task: "", constraints: "", judgeHints: "" };
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "loop_available":
          ...
          updateLoopButton();
          if (loopActive) showLoopBanner(true);
          break;
        ...
        case "ralph_phase":
          ...
          if (ralphPhase === "approval") {
            showRalphApprovalBar(true);
          } else if (ralphPhase !== "crafting") {
            showRalphApprovalBar(false);
          }
          break;
  ...
  function updateLoopButton() {
    ...
    var shouldShow = !loopActive && ralphPhase !== "executing";
    if (shouldShow) {
      if (!existing) {
        var btn = document.createElement("button");
        btn.id = "loop-start-btn";
        btn.className = "loop-start-btn";
        btn.textContent = "New Ralph Loop";
        btn.title = "Start a new Ralph Loop";
        btn.addEventListener("click", function() {
          if (loopAvailable) {
            // Files already exist — offer choice
            openRalphWizard();
          } else {
            openRalphWizard();
          }
        });
        var header = document.querySelector(".header-actions, .header-right, header");
  ...
  function showLoopBanner(show) {
    var existing = document.getElementById("loop-banner");
    if (!show) {
      if (existing) existing.remove();
      updateLoopButton();
      return;
    ...
    banner.id = "loop-banner";
    banner.className = "loop-banner";
    banner.innerHTML =
      '<div class="loop-banner-left">' +
        '<span class="loop-banner-icon">\u{1F501}</span> ' +
        '<span class="loop-banner-label">Ralph Loop</span> ' +
        '<span class="loop-banner-status" id="loop-status">Starting...</span>' +
      '</div>' +
      '<button class="loop-stop-btn" id="loop-stop-btn">Stop</button>';

    var messagesParent = messagesEl.parentNode;
    messagesParent.insertBefore(banner, messagesEl);

    document.getElementById("loop-stop-btn").addEventListener("click", function() {
      if (ws && ws.readyState === 1) {
  ...
  function openRalphWizard() {
    wizardStep = 1;
    wizardData = { task: "", constraints: "", judgeHints: "" };
    var el = document.getElementById("ralph-wizard");
    if (el) {
      el.classList.remove("hidden");
      updateWizardStep();
      var taskEl = document.getElementById("ralph-task");
      if (taskEl) { taskEl.value = ""; setTimeout(function() { taskEl.focus(); }, 100); }
      var constraintsEl = document.getElementById("ralph-constraints");
      if (constraintsEl) constraintsEl.value = "";
      var judgeEl = document.getElementById("ralph-judge-hints");
  ...
  function updateWizardStep() {
    ...
    if (skipBtn) skipBtn.style.display = (wizardStep === 1 || wizardStep === 4) ? "none" : "";
    if (nextBtn) nextBtn.textContent = wizardStep === 4 ? "Launch" : "Next";
    ...
    if (wizardStep === 4) {
      ...
      if (summary) {
        summary.innerHTML =
          ...
          '<div class="ralph-review-label">Constraints</div>' +
          '<div class="ralph-review-value">' + escapeHtml(wizardData.constraints || "(none)") + '</div>' +
          '<div class="ralph-review-label">Judge Hints</div>' +
          '<div class="ralph-review-value">' + escapeHtml(wizardData.judgeHints || "(none)") + '</div>';
      }
    }
  }
  ...
  function collectWizardData() {
    ...
    wizardData.constraints = constraintsEl ? constraintsEl.value.trim() : "";
    wizardData.judgeHints = judgeEl ? judgeEl.value.trim() : "";
  }

  function wizardNext() {
    collectWizardData();
    if (wizardStep === 1 && !wizardData.task) {
      var taskEl = document.getElementById("ralph-task");
  ...
  function showRalphCraftingBar(show) {
    var existing = document.getElementById("ralph-crafting-bar");
    if (!show) {
      if (existing) existing.remove();
      return;
    }
    ...
    bar.id = "ralph-crafting-bar";
    bar.className = "ralph-crafting-bar";
    bar.innerHTML =
      '<div class="ralph-crafting-left">' +
        '<span class="ralph-crafting-icon">\u2699\uFE0F</span>' +
        '<span class="ralph-crafting-text">Crafting PROMPT.md &amp; JUDGE.md</span>' +
        '<span class="ralph-crafting-sub">The skill is exploring your codebase...</span>' +
      '</div>' +
      '<button class="loop-stop-btn" id="ralph-cancel-crafting-btn">Cancel</button>';

    var messagesParent = messagesEl.parentNode;
    messagesParent.insertBefore(bar, messagesEl);

    document.getElementById("ralph-cancel-crafting-btn").addEventListener("click", function() {
      if (ws && ws.readyState === 1) {
  ...
  function showRalphApprovalBar(show) {
    var existing = document.getElementById("ralph-approval-bar");
    if (!show) {
      if (existing) existing.remove();
      return;
    }
    ...
    // Remove crafting bar when approval bar appears
    showRalphCraftingBar(false);

    var bar = document.createElement("div");
    bar.id = "ralph-approval-bar";
    bar.className = "ralph-approval-bar";
    bar.innerHTML =
      '<div class="ralph-approval-left">' +
        '<span class="ralph-approval-icon">\u2705</span>' +
        '<span class="ralph-approval-text" id="ralph-approval-text">PROMPT.md &amp; JUDGE.md ready</span>' +
      '</div>' +
      '<div class="ralph-approval-actions">' +
        '<button class="ralph-preview-btn" id="ralph-preview-btn">Preview</button>' +
        '<button class="ralph-start-btn" id="ralph-start-loop-btn">Start Ralph Loop</button>' +
      '</div>';

    var messagesParent = messagesEl.parentNode;
    messagesParent.insertBefore(bar, messagesEl);

    document.getElementById("ralph-preview-btn").addEventListener("click", function() {
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "ralph_preview_files" }));
      }
    });

    document.getElementById("ralph-start-loop-btn").addEventListener("click", function() {
      if (ws && ws.readyState === 1) {
  ...
  function updateRalphApprovalStatus() {
    var textEl = document.getElementById("ralph-approval-text");
    var startBtn = document.getElementById("ralph-start-loop-btn");
    if (!textEl) return;

    if (ralphFilesReady.bothReady) {
      textEl.textContent = "PROMPT.md & JUDGE.md ready";
      if (startBtn) startBtn.disabled = false;
    } else if (ralphFilesReady.promptReady) {
      textEl.textContent = "PROMPT.md ready, waiting for JUDGE.md...";
      if (startBtn) startBtn.disabled = true;
    } else if (ralphFilesReady.judgeReady) {
      textEl.textContent = "JUDGE.md ready, waiting for PROMPT.md...";
      if (startBtn) startBtn.disabled = true;
    } else {
      textEl.textContent = "Waiting for PROMPT.md & JUDGE.md...";
      if (startBtn) startBtn.disabled = true;
    }
  }
