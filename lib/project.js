...
function createProjectContext(opts) {
  ...
  function loadLoopState() {
    try {
      ...
      // If was executing, mark as needing recovery
      if (loopState.phase === "executing") {
  ...
  function checkLoopFilesExist() {
    var hasPrompt = false;
    var hasJudge = false;
    try { fs.accessSync(path.join(cwd, ".claude", "PROMPT.md")); hasPrompt = true; } catch (e) {}
    try { fs.accessSync(path.join(cwd, ".claude", "JUDGE.md")); hasJudge = true; } catch (e) {}
  ...
  function startClaudeDirWatch() {
    ...
    try {
      claudeDirWatcher = fs.watch(claudeDir, function () {
  ...
  function startLoop() {
    var promptPath = path.join(cwd, ".claude", "PROMPT.md");
    var judgePath = path.join(cwd, ".claude", "JUDGE.md");
    var promptText, judgeText;
    try {
      promptText = fs.readFileSync(promptPath, "utf8");
    } catch (e) {
      send({ type: "loop_error", text: "Missing .claude/PROMPT.md" });
      return;
    }
    try {
      judgeText = fs.readFileSync(judgePath, "utf8");
    } catch (e) {
      send({ type: "loop_error", text: "Missing .claude/JUDGE.md" });
      return;
    }
  ...
  function stopLoop() {
    ...
    var currentSession = loopState.currentSessionId != null
      ? sm.sessions.get(loopState.currentSessionId) || null : null;
    var judgeSession = loopState.judgeSessionId != null
      ? sm.sessions.get(loopState.judgeSessionId) || null : null;

    var sessionToAbort = (judgeSession && judgeSession.isProcessing)
      ? judgeSession : currentSession;

    if (sessionToAbort && sessionToAbort.abortController) {
      sessionToAbort.abortController.abort();
    }
  ...
  function handleConnection(ws) {
    ...
    if (loopState.phase === "crafting" || loopState.phase === "approval") {
      ...
      var _hasJudge = false;
      try { fs.accessSync(path.join(cwd, ".claude", "PROMPT.md")); _hasPrompt = true; } catch (e) {}
      try { fs.accessSync(path.join(cwd, ".claude", "JUDGE.md")); _hasJudge = true; } catch (e) {}
      sendTo(ws, {
        type: "ralph_files_status",
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "ralph_wizard_complete") {
      ...
      loopState.wizardData = {
        task: wData.task || "",
        constraints: wData.constraints || "",
        judgeHints: wData.judgeHints || "",
      };
      loopState.phase = "crafting";
      loopState.startedAt = Date.now();
      saveLoopState();

      // Assemble prompt for clay-ralph skill
      var craftingPrompt = "/clay-ralph\n## Task\n" + (wData.task || "") +
        "\n## Constraints\n" + (wData.constraints || "None") +
        "\n## Judge Hints\n" + (wData.judgeHints || "Use your judgment");
      ...
      send({ type: "ralph_crafting_started" });
      send({ type: "ralph_phase", phase: "crafting", wizardData: loopState.wizardData });
      return;
    }
    ...
    if (msg.type === "ralph_preview_files") {
      var promptContent = "";
      var judgeContent = "";
      try { promptContent = fs.readFileSync(path.join(cwd, ".claude", "PROMPT.md"), "utf8"); } catch (e) {}
      try { judgeContent = fs.readFileSync(path.join(cwd, ".claude", "JUDGE.md"), "utf8"); } catch (e) {}
      sendTo(ws, {
